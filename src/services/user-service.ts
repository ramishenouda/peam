...
export const GetUserProfile = async (username: string, token: string) => {
  ...
  options.url =
    baseURL +
    `user/profile/?expand=*&expand=courses_owned.owner&expand=courses_taken.owner&expand=courses_taught.owner`;
  options.method = 'GET';
...
export const UpdateAvatar = async (avatar: any, token: string) => {
  ...
  options.headers['Content-Type'] =
    'multipart/form-data; boundary=----WebKitFormBoundaryWP4KnN0qX6fToVf3';
