...
export const ListMembers = (props: Props) => {
  ...
  if (props.members) {
    members = props.members.map((item) => {
      ...
      return (
        <Div
          key={item.username + Date()}
          className={`px-3 pt-2 mb-2 text-dark bg-g-dark pb-5`}
        >
          <VerticalCenterdElement left={'10px'}>
            <img
              src={pictureURL}
              className="mr-2"
              alt="teacher"
          ...
          {props.showButton && (
            <VerticalCenterdElement right={'10px'}>
              <Button
                onClick={() => {
                  props.optionFuncton && props.optionFuncton(item);
                }}
                variant="dark"
              >
                {props.ButtonText}
  ...
  return (
    <div>
      {members ? (
        ...
        <div>
          Error while loading the teachers.
          <br />
          Please reloard the page.
        </div>
      )}
    </div>
  );
