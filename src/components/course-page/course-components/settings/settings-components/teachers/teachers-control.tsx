...
import { success, confirm } from '../../../../../../services/notification-service';
import { DeleteCourseTeacher, GetCourseTeachers } from '../../../../../../services/course-service';
...
export const TeachersControler = (props: Props) => {
    ...
    const remove = (username: string, name: string) => {
        confirm('', `Are you sure you want to remove </br> <strong> 
            ...
            DeleteCourseTeacher(courseState.owner, courseState.code, username, systemState)
                .then((result) => {
                    success('Student was removed successfully');
                    setTeachers(prevState => prevState.filter(x => x.teacher.username !== username));
                }).catch((err) => {
                    showAxiosResponseErrors(err);
                });
        })
    ...
    if (fetching) {
                ...
                <div>
                    <Div className=" p-3 my-4">
                        ...
                        <div className="remove-btn float-right">
                            {
                                (systemState.username === courseState.owner && systemState.username !== teacher.username) && <Button variant="danger" onClick={() => remove(teacher.username, teacher.name)}>Remove</Button>
                            }
                        </div>
                        <Div></Div>
                    </Div>
        ...
        return (
            <div className="text-center f1 mt-2">
                <hr/>
                <Title className="text-left f1">
                    Manage current teachers
                </Title>
               { data }
            </div>
        );
