...
export const Courses = (props: Props) => {
  ...
  const view = data.courses_owned.map((item: any) => {
    return (
      <Div key={item.title + Date()} className="p-3 mt-2">
        <Link
          to={`/${item.owner.username}/${item.code}`}
          className="disable-link-style link f1"
        >
          <Title>{item.title}</Title>
        </Link>
        <Description className="f2">{item.description}</Description>
      </Div>
    );
  });
  ...
  return (
    <div>
      <Title className="f2">Owned courses</Title>
      {view}
      <hr />
      <Title className="f2">Taught courses</Title>
      {coursesAsTeacher}
      <hr />
      <Title className="f2">Taken courses</Title>
      {coursesAsStudent}
    </div>
  );
};
