...
import { DeleteIcon, Div } from '../../style';
...
export const PendingInvitationItem = (props: Props) => {
  ...
  const type = props.invitation.type ? props.invitation.type : 'member';

  const deleteInvitation = () => {
    confirm(
      'Are you sure?',
      "You won't be able to restore this attachment"
    ).then((result) => {
      ...
      setDeleting(true);
      if (props.removeInvite) {
        props
          .removeInvite(props.invitation.token, props.token)
          .then(() => {
  ...
  return (
    <Div className="p-3 mb-2 text-dark bg-g-dark">
      <div className={`${!mdScreen && 'float-left'} link`}>
        ...
        <span className="px-2 py-1">{props.invitation.email}</span>
      </div>
      <div
        className={`text-light ${
          !mdScreen ? 'float-right' : 'mt-3 text-right'
        }`}
      >
        {status === 'Pending' && !deleting && (
          <Status className="bg-info px-2 py-1 text-bold">PENDING</Status>
        )}
        {status === 'Accepted' && !deleting && (
          <Status className="bg-success px-2 py-1 text-bold">ACCEPTED</Status>
        )}
        {status === 'Rejected' && !deleting && (
          <Status className="bg-danger px-2 py-1 text-bold">REJECTED</Status>
        )}
